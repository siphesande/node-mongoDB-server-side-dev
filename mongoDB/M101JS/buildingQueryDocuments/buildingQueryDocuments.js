...
function queryDocument(options) {
    ...
    if (("firstYear" in options) && ("lastYear" in options)) {
        ...
        */
        ...
        // } }
       //query.founded_year = {$and: [{ "$gte": options.firstYear },{ "$lte": options.lastYear }]}
        query.founded_year = {
                            "$gte": options.firstYear,
                            "$lte": options.lastYear
                             } 
    ...
    if ("city" in options) {
        ...
        */    
         //query["offices.city"] /* city.offices*/ = {"$regex": options.city, "$options": "i"};
         //query.city = {"$regex": options.city, "$options": "i"};
...
function report(options) {
    ...
    console.log("Average number of employees per company: " + Math.floor(totalEmployees / companiesList.length));//169
}
